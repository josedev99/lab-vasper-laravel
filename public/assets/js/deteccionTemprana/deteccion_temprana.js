...
//agregar nuevo grupo 
       ...
       let btnAddDeteccion = document.querySelector('.btnAddGrupo');
       if (btnAddDeteccion) {
           btnAddDeteccion.addEventListener('click', (event) => {
               event.preventDefault();
               $("#modal_form_new_deteccion").modal('show');
           });
       }
    ...
    try {
        ...
        if (formAddItem) {
            formAddItem.addEventListener('submit', (event) => {
                ...
                if (index !== -1) {
                    ...
                    $("#modal_new_examen_add").modal('hide');
                }
            })
        }
        ...
        if (form_gru_o) {
            form_gru_o.addEventListener('submit', (event) => {
                ...
                axios.post(route('dteccion.save'), formData)
                    .then((result) => {
                        ...
                        if (result.data.status === "success") {
                            ...
                            $("#modal_form_new_deteccion").modal('hide');
                            form_gru_o.reset();
                            //add item factor
        ...
        let btnSaveChangePerfil = document.getElementById('btnSaveChangePerfil11');
...
function guardarDatos() {
    ...
    if (array_perfil_riesgo.length > 0) {
        ...
        formData.append('data_detecc', JSON.stringify(array_perfil_riesgo));
        console.log(array_perfil_riesgo)
        axios.post(route('deteccion.exam.save'), formData)
            .then((result) => {
                let message = result.data.message;
                if (result.data.status === 'success') {
                    Toast.fire({
...
function loadPerfil() {
    let component_body_deteccion = document.getElementById('component_body_deteccion');
    component_body_deteccion.innerHTML = '';
    ...
    if (array_perfil_riesgo.length > 0) {
        array_perfil_riesgo.forEach((item, index) => {
            ...
            if (item.removeStatus === false) {
                ...
                component_body_deteccion.appendChild(rowItem); // Agregamos el tr al cuerpo de la tabla
            }
        });
    } else {
        component_body_deteccion.innerHTML = '<tr><td colspan="3" class="text-center">Sin datos para mostrar.</td></tr>';
    }
...
function addItemExamen(element) {
    ...
    $("#modal_new_examen_add").modal('show');
}
//add item array
...
function get_data_perfil() {
    ...
    axios.post(route('deteccion.data'))
        .then((result) => {
